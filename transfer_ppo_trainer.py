...
import numpy as np
import torch
from habitat_baselines.utils.common import batch_obs, linear_decay, generate_video
...
from habitat_baselines.rl.ppo import ppo as ppo
from habitat_baselines.rl.ppo import policy as policy
# import transfer_learn as transfer_learn
...
class PPOTrainer(BaseRLTrainer):
    ...
    def _setup_actor_critic_agent(self, ppo_cfg):
        ...
        self.actor_critic = policy.PointNavBaselinePolicy(
    ...
    def _collect_rollout_step(self, rollouts, current_episode_reward, episode_rewards, episode_counts, episode_dist):
        pth_time = 0.0
        env_time = 0.0
    ...
    def train(self, env) -> None:
        ...
        # transfer_cnn = transfer_learn.train_cnn(self.env, self.actor_critic)
        # for i in range(5):
        #     self.actor_critic._modules["net"].visual_encoder._modules["cnn"][i] = transfer_cnn[i]
        ...
        for update in range(self.config.NUM_UPDATES):
            ...
            for step in range(ppo_cfg.num_steps):
                (
                    ...
                    delta_steps,
                    tmp_dist,
                ) = self._collect_rollout_step(rollouts, current_episode_reward, episode_rewards, episode_counts, episode_dist,)
        ...
        np.savetxt("episode_dist_ppo.csv", dist_val, delimiter=",")
