...
class RolloutStorage:
    ...
    def __init__(self, num_steps, num_envs, observation_space, action_space, recurrent_hidden_state_size, num_recurrent_layers=1):
        ...
        self.recurrent_hidden_states = torch.zeros(num_steps + 1, num_recurrent_layers, num_envs, recurrent_hidden_state_size)
        ...
        self.action_log_probs = torch.zeros(num_steps, num_envs, len(action_space.spaces.keys())-1)

        if action_space.__class__.__name__ == "ActionSpace":
            action_shape = 1
        else:
            action_shape = action_space.shape[0]-1
    ...
    def insert(self, observations, recurrent_hidden_states, actions, action_log_probs, value_preds, rewards, masks):
    ...
    def recurrent_generator_dqn(self, num_mini_batch):
        num_processes = self.rewards.size(1)
        assert num_processes >= num_mini_batch, (
            "Trainer requires the number of processes ({}) "
            "to be greater than or equal to the number of "
        ...
        for start_ind in range(0, num_processes, num_envs_per_batch):
            ...
            recurrent_hidden_states_batch = []
            next_recurrent_hidden_states_batch = []
            actions_batch = []
            state_batch = defaultdict(list)
            next_state_batch = defaultdict(list)
            masks_batch = []
            next_masks_batch = []
            reward_batch = []
            ...
            for offset in range(num_envs_per_batch):
                ...
                for sensor in self.observations:
                    next_state_batch[sensor].append(self.observations[sensor][1: self.step, ind])
                    state_batch[sensor].append(self.observations[sensor][: self.step, ind])

                for sensor in self.observations:
                    next_state_batch[sensor][0] = torch.cat([next_state_batch[sensor][0], torch.zeros(next_state_batch[sensor][0][:1, :, ].shape).to(
                        device=state_batch[sensor][0].device)], dim=0)


                recurrent_hidden_states_batch.append(self.recurrent_hidden_states[0, :, ind])
                next_recurrent_hidden_states_batch.append(self.recurrent_hidden_states[1, :, ind])

                actions_batch.append(self.actions[: self.step, ind])
                masks_batch.append(self.masks[: self.step, ind])
                next_masks_batch.append(self.masks[1: self.step, ind])
                next_masks_batch[0] = torch.cat([next_masks_batch[0], torch.zeros(next_masks_batch[0][:1, :, ].shape).to(device=next_masks_batch[0].device)], dim=0)
                reward_batch.append(self.rewards[: self.step, ind])


            T, N = self.step, num_envs_per_batch

            # These are all tensors of size (T, N, -1)
            for sensor in state_batch:
                state_batch[sensor] = torch.stack(state_batch[sensor], 1)
                next_state_batch[sensor] = torch.stack(next_state_batch[sensor], 1)

            actions_batch = torch.stack(actions_batch, 1)
            reward_batch = torch.stack(reward_batch, 1)
            masks_batch = torch.stack(masks_batch, 1)
            next_masks_batch = torch.stack(next_masks_batch, 1)

            # States is just a (num_recurrent_layers, N, -1) tensor
            recurrent_hidden_states_batch = torch.stack(recurrent_hidden_states_batch, 1)
            next_recurrent_hidden_states_batch = torch.stack(next_recurrent_hidden_states_batch, 1)

            # Flatten the (T, N, ...) tensors to (T * N, ...)
            for sensor in state_batch:
                state_batch[sensor] = self._flatten_helper(T, N, state_batch[sensor])
                next_state_batch[sensor] = self._flatten_helper(T, N, next_state_batch[sensor])

            actions_batch = self._flatten_helper(T, N, actions_batch)
            reward_batch = self._flatten_helper(T, N, reward_batch)
            masks_batch = self._flatten_helper(T, N, masks_batch)
            next_masks_batch = self._flatten_helper(T, N, next_masks_batch)


            yield (state_batch, actions_batch, next_state_batch, reward_batch, masks_batch, recurrent_hidden_states_batch, next_masks_batch, next_recurrent_hidden_states_batch)
